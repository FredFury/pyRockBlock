...
# Original code here: https://github.com/MakerSnake/pyRockBlock
#
# Converted for Python3 compatibility:
# Ensure all serial comms out is encoded and serial comms in is decoded
#
# Added some print strings to explain why message sending fails
#
...
class rockBlock(object):
    ...
    def sendMessage(self, msg):
        ...
        print("Check connection and message queue")
    ...
    def _queueMessage(self, msg):
        ...
        if(self.s.readline().decode().strip() == command):
            ...
            if(self.s.readline().decode().strip() == "READY"):
                print("Modem ready to transmit message: " + msg)
                checksum = 0
                ...
                if(response == "1"):
                    result = False
                    print("SBD message write timeout. An insufficient number of bytes were transferred to ISU during the transfer period of 60 seconds.")

                if(response == "2"):
                    print("SBD message checksum sent from DTE does not match the checksum calculated at the ISU.")
                    result = False
                
                if(response == "3"):
                    print("SBD message size is not correct.")
                    result = False
                ...
                return result
        print("No echo from modem detected")
        return False
